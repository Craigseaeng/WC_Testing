...
const unsigned long updateInterval = 900*1000; // 900 = 15 minutes: 60 seconds in milliseconds 
...
void sendStatusMessage(const String &statusMessage) {
    // Send the status message over BLE
    if (deviceConnected) {
        pTxCharacteristic->setValue(statusMessage.c_str());
        pTxCharacteristic->notify();
        Serial.println("Sent Status: " + statusMessage);
    } else {
        Serial.println("BLE client not connected. Status not sent.");
    }
}
...
void startOTAUpdate() {
    ...
    // Declare a secure client for HTTPS
    WiFiClientSecure client;
    client.setInsecure();      // Disable SSL verification for testing purposes
    client.setTimeout(5000); 

    HTTPClient http;           // HTTPClient to handle the request

    String firmwareURL = "https://raw.githubusercontent.com/Craigseaeng/WC_Testing/main/firmware.bin";  // Corrected URL

    Serial.print("Requesting firmware from: ");
    Serial.println(firmwareURL);

    http.begin(client, firmwareURL);  // Begin the connection with the URL
    int httpCode = http.GET();        // Make the request to get the file

    if (httpCode == HTTP_CODE_OK) {
        // Get the size of the file from the response header
        unsigned long firmwareSize = http.getSize();
        ...
        if (firmwareSize > 0) {
            ...
            sendStatusMessage("Starting OTA update...");

            delay(2000);

            WiFiClient *stream = http.getStreamPtr();  // Get the stream to download the file

            // Begin the OTA process with the dynamically retrieved firmware size
            Update.begin(firmwareSize);  // Indicate the size of the incoming update
            size_t written = Update.writeStream(*stream);  // Write the data to flash memory

            if (written == firmwareSize) {
                if (Update.end()) {
                    sendStatusMessage("OTA Update Success!");
                    Serial.println("Update successfully completed. Rebooting...");
                    

                    ESP.restart();  // Reboot the ESP32 to apply the new firmware
                } else {
                    Serial.println("Update failed.");
                    sendStatusMessage("OTA Update Failed!");
                }
            } else {
        ...
        Serial.print("Failed to get update, HTTP error: ");
        Serial.println(httpCode);  // Print the HTTP error code if the request fails
        Serial.println(http.errorToString(httpCode));  // Show a detailed error message
        sendStatusMessage("OTA Update Failed!");
    }
...
//        sendStatusMessage("Received BLE Data: " + rxValue);
        delay(3000);
//        sendStatusMessage("Received BLE Data: " + rxValue);
        ...
        if (rxValue.startsWith("OTA")) {
            ...
            if (deviceConnected) {
                pTxCharacteristic->setValue("Triggering OTA");
                pTxCharacteristic->notify();
            }
            delay(10000);

            // Start OTA process without needing the size argument
            startOTAUpdate();  // Call the function without the size argument
        }
        // Handle WiFi credentials update
        else if (rxValue.length() > 0) {
              // Remove any blank spaces from the incoming string
            rxValue.replace(" ", "");
...
void fetchData() {
    ...
    WiFiClientSecure client;
    client.setInsecure();
    client.setTimeout(5000);  // Let handshake stabilize
    HTTPClient https;
...
void initializeWiFi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        sendStatusMessage("WiFi Connected!");
        delay(2000); // Give the network stack a moment to stabilize
    } else {
        Serial.println("WiFi Connection Failed!");
        Serial.print("Status code: ");
        Serial.println(WiFi.status());
        sendStatusMessage("WiFi Connection Failed!");
    }
}
...
void loop() {
    ...
    if (millis() - lastUpdateTime >= updateInterval) {
        ...
        initializeWiFi(); // Reinitialize WiFi with credentials
